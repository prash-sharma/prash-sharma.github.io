        ...
        <>
            ...
            <Confirmation 
                ...
                onCancel = {()=>{
                    setTriggerDel(false);
                    setBgScroll(false)
                }}>Lacks firepower, let's kick <b>{memberName}</b> out!!</Confirmation>

            <div className='memberDiv' onMouseEnter={(e)=>{setOperation(true); e.preventDefault()}} onMouseLeave={()=>{setOperation(false)}}>
                <div className={'memberDetails'}>
                    <div>
                        <h3>Name:</h3> <p>{memberName}</p>
                        <h3>Expertise:</h3> <p>{memberExpertise}</p>
                        <h3>From:</h3><p>{memberFrom}</p>
                    </div>    
                    <div>
                        <img src={`${memberImage}`} alt = {`${memberName}`} width={200} height={200}/>
                    </div>
                </div>    
                <div className='operationDiv'>
                    { operation && (
                        <> 
                            <Link to={`/update/${memberId}`}><img src={editIcon} alt={'editIcon'} style={operationStyle}/></Link>
